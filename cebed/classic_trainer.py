...
class TrainConfig:
    ...
    # configs that cannot be changed!
    dataset_name: str = "Classic" # MUST BE "Classic"
    main_input_type: str = "low"
    aux_input_type: str = "low"
    task: str = "main" # MUST BE "main"

    # data-related
    data_dir: str = "../data/ps2_p72/rayleigh/snr0to25_speed5"
    train_split: float = 0.6

    # model-class-related: models
    experiment_name: str = ""
    model_name: str = "HA02" # "HA02" or "ChannelNet"
    verbose: int = 1
    output_dir: str = "offline_train_output"
...
class Trainer:
    ...
    def __init__(self, config: TrainConfig):
        ...
        self.run_name = f"bs{self.config.train_batch_size}_lr{self.config.lr}_snr{self.start_snr}to{self.end_snr}_{self.config.model_name}"
...
if __name__ == "__main__":
    ...
    # -------------------------------- TrainConfig ------------------------------- #
    # The following arguments are exactly the same as the dataclass TrainConfig in cebed/TTT/trainer.py
    import argparse
    parser = argparse.ArgumentParser(description="Channel Estimation Offline Pretraining")

    ########################## General args ###################################
    parser.add_argument("--experiment_name", type=str, default = "siso_1_umi_block_1_ps2_p72")
    parser.add_argument("--data_dir", type=str, default="./data/ps2_p72/uma/snr10to25_speed5")
    ...
    parser.add_argument("--early_stopping", default=True, action="store_true", help="If early stopping should be used"
    )

    ## MODEL!!
    parser.add_argument("--model_name", type=str, default="ReEsNet") # "HA02" or "ChannelNet" or "ReEsNet"
