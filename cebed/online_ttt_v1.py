...
This file includes the logic of online TTT based on a pretrained two-branch model. [Key function: online_adapt(..)]
-> Before TTT, we first do an evaluation on the pretrained model with online test data; Log the results as Epoch 0
-> During TTT [REQUIRE: "--ssl" = 1, "--supervised" = 0]
    1) load a batch of test data (use "--sym_error_rate" to artifically create random symbol estimation errors for Aux-Task, common values:0.0005,0.001,0.002,0.005,0.01,...);
    2) augment this batch (TODO: use the same logic as "generate_aux_data_online(...)")
    3) train the model [enter the ssl mode, and set the trainable parameters]
-> Upper bound: abandon TTT and do online supervised learning for main-task. This is under the idealistic assumption that ground-truth channels
  (i.e., main-task labels) are known. [REQUIRE: "--ssl" = 0, "--supervised" = 1]
   The supervised learning always lead to the best performance of channel MSE, which is deemed as the upper bound.
-> Lower bound: do not do any online training, just use the pretrained model for testing  [REQUIRE: "--ssl" = 0, "--supervised" = 0]

Other Notes:
-> in this file, "--aug_noise_std" should be set as 0 unless one wants to vary the noise level in data augmentation.
    ...
    def online_adapt(self,snr_range: List[int]) -> None:
        ...
        for i,snr in enumerate(snr_range):
            ...
            if snr == 20:
                ...
                before_adapt_test_data = iter(self.loader)
                ...
                wandb.log({'epoch': 0,
                        'epoch_channel_mse': np.mean(before_adapt_channel_mse),
                        'epoch_aux_loss': np.mean(before_adapt_aux_mse)})        
                ...
                for epoch in range(self.config.epochs):
                    ...
                    for step in range(num_steps):
                        ...
                        # # whether fully share the model between two tasks
                        # if epoch==0 and step==0:
                        #     pass # remain the pretrained model
                        # else:
                        #     self.model.main_decoder.set_weights(self.model.aux_decoder.get_weights())
                        ...
                        with tf.GradientTape() as tape:
                            ...
                            sum_loss = self.model.compiled_loss(h_true, h_pred) + self.model.compiled_loss(y_aux, aux_pred)
...
if __name__ == "__main__":
    ...
    ########################## General args ###################################
    parser.add_argument("--trained_model_dir", type=str, default="./main4_aux2_weights_ds_discrete_10-25.h5") # uma_speed5
    parser.add_argument("--eval_data_dir", type=str, default="./data/ps2_p72/rma/snr10to11_speed30") # related to OOD test dataset
