...
class Evaluator:
    ...
    def __init__(self, config: EvalConfig):
        ...
        self.model_name = "DnCNN" # fix mask for main, random mask for aux
    ...
    def setup(self):
        ...
        assert self.config.eval_dataset_name == "Denoise" # MUST be RandomMask for adapt-task
        # Construct data directory in online_adapt(), here is just for initialization
        print(self.config.train_data_dir)
    ...
    def _evaluate_pre_adapt(self, scenario: str, speed: str, scenario_name: str):
        ...
        for snr in tqdm(snr_range, desc=f"Pre-adapt {scenario_name} SNR Evaluation"):
            ...
            for step in tqdm(range(num_steps), desc=f"Model Evaluation SNR {snr}", leave=False):
                (h_ls, h_true), _ = next(eval_data_iterator)

                # bilinear interpolation to the model input
                h_ls_interp = tf.image.resize(h_ls, [14, 72], method=tf.image.ResizeMethod.BILINEAR)
                h_pred = self.model(h_ls_interp)
                # h_pred: [batch, 14, 72, 2]
    ...
    def _perform_adapt_training(self, total_samples):
        ...
        for epoch in tqdm(range(self.config.epochs), desc="Adaptation Training Epochs"):
            ...
            for step in tqdm(range(num_steps), desc=f"Epoch {epoch+1} Training Steps", leave=False):
                ...
                # Get batch data
                (h_ls, h_true), (y_no, y_true)= next(batch_data)
                ...
                with tf.GradientTape() as tape:
                    ...
                    y_pred = self.model(y_no)
                    main_loss = self.model.compiled_loss(h_true, h_pred)
                    train_loss = self.model.compiled_loss(y_true, y_pred) # online denoising task loss
                
                # Cast tensors to same dtype to avoid dtype mismatch
                h_true = tf.cast(h_true, tf.float32)
                h_pred = tf.cast(h_pred, tf.float32)
                y_no = tf.cast(y_no, tf.float32)
                y_pred = tf.cast(y_pred, tf.float32)
                    
                step_main_mse = mse(h_true, h_pred).numpy()
                step_denoise_mse = mse(y_true, y_pred).numpy()
                channel_mse.append(step_main_mse)
                denoise_mse.append(step_denoise_mse)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="experiment_results/dncnn")
