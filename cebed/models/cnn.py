...
class ChannelNet(BaseModel):
    ...
    def call(self, inputs):
        ...
        latent_x = self.sr_model(x)
        # input x.shape = [batch_size, nps, npf, 2] 
        # output x.shape = [batch_size, num_symbol, num_subcarrier, 2] 
        
        # TODO: what if just discard sr_model()??? can it work???
        noise = self.denoiser(x)
        # noise.shape = [batch_size, num_symbol, num_subcarrier, 2]

        # in our setting (dc_null = False, guard_band = [0,0]),
        # so that num_subcarrier = num_effective_subcarrier
