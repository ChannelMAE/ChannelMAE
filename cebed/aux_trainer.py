"""
Code script to train a model on aux task: received signal reconstrcution
"""
import os
...
class AuxTrainer:
    ...
    def __init__(self, config: AuxTrainConfig):
        self.config = config
        ...
        self.run_name = f"bs{self.config.train_batch_size}_lr{self.config.lr}_ser{self.config.sym_error_rate}_snr{self.start_snr}to{self.end_snr}_{self.config.model_name}_dl=3"
    ...
    def setup(self):
        ...
        if self.config.dataset_name == "Denoise":
        ...
        elif self.config.dataset_name == "FixMask" or "RandomMask":
            self.dataset = dataset_class(
                self.config.data_dir,
                train_split=self.config.train_split,
                main_input_type=self.config.main_input_type,
        ...
        # print(input_shape_aux)

        # build the model
        if self.config.model_name =="MaeRandomMask":
        ...
        elif self.config.model_name == "MaeFixMask": # TODO: to use this model, we must use ""FixMask" dataset
