...
class BaseDataset:
    ...
    def __init__(
        ...
        self.train_x_aux = None
        self.train_y_aux = None
        self.val_x_aux = None
        self.val_y_aux = None
        self.test_x_aux = None
    ...
    def split(self):
        ...
        for ds in range(self.num_domains):
            ...
            # aux task
            train_x_aux.append(self.inputs_aux[ds][train_indices])
            train_y_aux.append(self.labels_aux[ds][train_indices])
            val_x_aux.append(self.inputs_aux[ds][val_indices])
            val_y_aux.append(self.labels_aux[ds][val_indices])
            test_x_aux.append(self.inputs_aux[ds][test_indices])
        ...
        self.train_x_aux = np.array(train_x_aux)
        self.train_y_aux = np.array(train_y_aux)
        self.val_x_aux = np.array(val_x_aux)
        self.val_y_aux = np.array(val_y_aux)
        self.test_x_aux = np.array(test_x_aux)
    ...
    def preprocess_aux(self, input, label, train=True):
        if train == True:
            label = self.preprocess_labels_aux(label)
        input = self.preprocess_inputs_aux(input, self.aux_input_type, self.mask)
        return input, label
    # ---------------------------------------------------------------------------- #
    ...
    # ----------------------------- Get data loaders ----------------------------- #
    # Note that we do preprocessing when creating dataloaders (i.e.,tf.data.Dataset)
    def get_train_loader(self, batch_size:int, task: str = "both") -> tf.data.Dataset:
        ...
        main_ds = main_ds.map(self.preprocess_main, num_parallel_calls=tf.data.AUTOTUNE) # work on each element/sample

        # create aux-task dataset and preprocess
        train_datasets = []
        for i in range(self.num_domains):
            ds = tf.data.Dataset.from_tensor_slices((self.train_x_aux[i], self.train_y_aux[i]))
            train_datasets.append(ds)
        ...
        if task == "both":
            ...
            train_ds = tf.data.Dataset.zip((main_ds, aux_ds)) # ((train_x_main, train_y_main),(train_x_aux, train_y_aux))
            # prepare the dataset, do not need any preprocessing here; shuffling
    ...
    def get_eval_loader(self, batch_size:int =32, setname: str = 'val', task: str = "both") -> tf.data.Dataset:
        ...
        # --------------------------------- aux task --------------------------------- #
        inputs_aux = combine_datasets(inputs_aux)
        ...
        aux_eval_ds = tf.data.Dataset.from_tensor_slices((inputs_aux, labels_aux))
