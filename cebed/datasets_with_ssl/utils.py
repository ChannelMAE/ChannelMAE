...
###############################################################################
def flip_sign(x):
    choice = np.random.choice(['real', 'imag', 'both'])
    if choice == 'real':
        x = x.real * -1 + 1j * x.imag
    elif choice == 'imag':
        x = x.real + 1j * x.imag * -1
    elif choice == 'both':
        x = x.real * -1 + 1j * x.imag * -1
    return x


def complex_normal(mean = 0.0, std = 1.0, shape = None):
    """
    Generate a complex normal distribution
    """
    real = np.random.normal(mean, std, shape).astype(np.float32)
    imag = np.random.normal(mean, std, shape).astype(np.float32)
...
def make_split(data_size, train_fraction=0.9):
    ...
    np.random.shuffle(indices) # shuffle test/train indices, so that the test indices should be randomly test_fraction
...
def postprocess(inputs: tf.Tensor) -> tf.Tensor:
    # From real inputs to complex, for each sample
    inputs = real_to_complex(inputs)    # [14, 72, 2] --> [14, 72, 1]
